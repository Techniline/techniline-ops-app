...
/** Manager tool: verify the Amazon SP-API credentials and probe which Vendor
 *  APIs / reports the granted roles allow. */
export function SpapiConnectionCheck() {
  ...
  async function test() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/spapi/ping", { headers: await authHeader() });
