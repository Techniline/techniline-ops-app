...
export function SkuCostsModal({
  ...
  const [qSku, setQSku] = useState(initialSku ?? "");
  const [qExpected, setQExpected] = useState("");
  const [showQuickAdd, setShowQuickAdd] = useState(!!initialSku);
  ...
  useEffect(() => {
    if (showQuickAdd && qSku.trim()) {
      setTimeout(() => qExpRef.current?.focus(), 50);
    }
  }, [showQuickAdd, qSku]);
  ...
  function setCell(sku: string, val: string) {
  ...
  async function quickSave() {
    ...
    const expected_in_hand = qExpected.trim() === "" ? null : Number(qExpected);
    if (qExpected.trim() !== "" && !Number.isFinite(expected_in_hand)) { setErr("Enter a valid number."); return; }
  ...
  async function removeRow(sku: string) {
    setBusy(true);
      ...
      <div onClick={(e) => e.stopPropagation()} className="flex max-h-[85vh] w-full max-w-xl flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        ...
        {showQuickAdd ? (
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg bg-slate-50 p-2 dark:bg-slate-800/60">
            ...
            <button type="button" onClick={() => openQuickAdd()} disabled={busy} className={`${btnSecondary} ml-auto`}>+ Add SKU</button>
          </div>
        )}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} className="px-2 py-6 text-center text-slate-400">
                  {search.trim()
                    ? <><span>No match for &quot;{search.trim()}&quot; — </span><button type="button" onClick={() => openQuickAdd(search)} className="font-medium text-indigo-600 hover:underline dark:text-indigo-400">add it now ↗</button></>
                    : "No SKUs yet — import a spreadsheet or add one."}
                </td></tr>
              ) : shown.map((r) => (
                <tr key={r.seller_sku} className="border-t border-slate-200/70 dark:border-slate-800">
                  <td className="px-2 py-1 font-medium">{r.seller_sku}</td>
                  <td className="px-2 py-1">
                    <input type="number" step="0.01" className={`${inputClass} !py-1 text-right`} value={r.expected}
                      onChange={(e) => setCell(r.seller_sku, e.target.value)} />
                  </td>
                  <td className="px-2 py-1 text-right">
                    <button type="button" onClick={() => void removeRow(r.seller_sku)} className="text-slate-400 hover:text-rose-600" title="Delete">🗑</button>
                  </td>
                </tr>
              ))}
            </tbody>
