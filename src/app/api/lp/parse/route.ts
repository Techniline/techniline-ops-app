...
import { parseLpPdf } from "@/lib/lp/parseLp";
...
/**
 * Auto-capture a Local Purchase Order from an uploaded PDF. Returns a draft for
 * the user to verify/edit before saving — this endpoint performs NO database
 * write. Authenticated app users only (the AI extraction call costs money).
 */
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const { draft, engine } = await parseLpPdf(bytes);
    return Response.json({ ok: true, draft, engine });
  } catch (error) {
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : "Failed to parse the LP." },
      { status: 500 }
    );
  }
}
