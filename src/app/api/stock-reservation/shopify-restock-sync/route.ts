...
function credentialsConfigured(): boolean {
      return Boolean(shopDomain() && process.env.SHOPIFY_ADMIN_TOKEN);
}

/**
 * Returns the static Shopify Admin API access token used elsewhere in this
 * app (see src/lib/shopify/client.ts). Custom-app tokens don't need a
 * separate OAuth exchange.
 */
...
const VARIANT_BY_SKU_QUERY = `
  query VariantBySku($query: String!, $first: Int!) {
...
/**
 * Looks up many SKUs at once by combining them into a single OR'd Shopify
 * search query per chunk, instead of one GraphQL round-trip per SKU. This is
 * what keeps the sync well under Vercel's function timeout when there are
 * many outstanding SKUs (previously: one sequential awaited request per SKU,
 * which is what caused the 300s FUNCTION_INVOCATION_TIMEOUT).
 */
...
export async function POST(request: Request): Promise<Response> {
  ...
  const variantsBySku = await findVariantsBySkus(token, [...outstanding.keys()], errors);

  for (const [sku, eta] of outstanding) {
          const variant = variantsBySku.get(sku) ?? null;
