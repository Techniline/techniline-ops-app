...
import { spapiConfigured, spapiPing, marketplaceId } from "@/lib/spapi/client";
...
/** Manager-only: verify the Amazon SP-API credentials are configured & working. */
export async function GET(request: Request): Promise<Response> {
  if (!(await isManagerRequest(request))) {
    return Response.json({ ok: false, error: "Unauthorized (manager only)." }, { status: 401 });
  }
  if (!spapiConfigured()) {
    return Response.json({ ok: false, configured: false, error: "SP-API env vars not set." });
  }
  const ping = await spapiPing();
  return Response.json({ ok: ping.ok, configured: true, marketplaceId: marketplaceId(), detail: ping.detail });
}
