...
import { sellerConfigured, sellerMarketplaceId, sellerPing, sellerProbe } from "@/lib/spapi/sellerClient";
...
/** Manager-only: verify the Seller SP-API creds and (optionally) probe roles. */
export async function GET(request: Request): Promise<Response> {
  if (!(await isManagerRequest(request))) {
    return Response.json({ ok: false, error: "Unauthorized (manager only)." }, { status: 401 });
  }
  if (!sellerConfigured()) {
    return Response.json({ ok: false, configured: false, error: "Seller SP-API env vars not set." });
  }
  const probe = new URL(request.url).searchParams.get("probe") === "1";
  const ping = await sellerPing();
  return Response.json({
    ok: ping.ok,
    configured: true,
    marketplaceId: sellerMarketplaceId(),
    detail: ping.detail,
    ...(probe ? { results: await sellerProbe() } : {}),
  });
}
