...
export async function POST(request: Request): Promise<Response> {
  ...
  let b: { subject?: unknown; html?: unknown };
  try {
    b = await request.json();
  } catch {
    return Response.json({ ok: false, error: "Invalid JSON." }, { status: 400 });
  }
  const subject = typeof b.subject === "string" && b.subject ? b.subject : "Marketplace return — documentation needed";
  const html = typeof b.html === "string" && b.html.trim() ? b.html : "<p>A marketplace return needs documentation.</p>";
