...
export default function PackingListView({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleExcelExport() {
    ...
    const res = await fetch(`/api/packing/lists/${id}/excel`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) { alert("Excel export failed."); return; }
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = res.headers.get("Content-Disposition")?.match(/filename="([^"]+)"/)?.[1] ?? "packing-list.xlsx";
    a.click();
