...
**Cocoblu PDF invoice capture shipped 2026-06-07 (`8026458`, free-parser default in follow-up):** `/cocoblu` → **Upload Invoice (PDF)** → server route `/api/cocoblu/parse` (unpdf text extract) auto-captures invoice header + line items → editable **Verify** modal → **Verify & Save** writes one `cocoblu_ageing` row per line, stores the PDF, records source/verified_by/verified_at. Managers get an **Edit** button on saved records; 📎 opens the stored PDF. **Capture engine: free built-in parser by default (no key/cost); auto-upgrades to AI (Claude Sonnet 4.6) when `ANTHROPIC_API_KEY` is set — no code change.** Deps: `@anthropic-ai/sdk`, `unpdf`. **Owner setup before it can save — see [COCOBLU-INVOICE-SETUP.md](COCOBLU-INVOICE-SETUP.md): (1) `cocoblu-invoices` storage bucket + policies, (2) 4 audit columns on `cocoblu_ageing`. (Optional later: `ANTHROPIC_API_KEY` to enable AI capture.)** Fails safe until then.
